...
def _generate_input_output(
    config: DictConfig, num_haystack: int, haystack: list[str]
) -> tuple[str, list[str]]:
    keys, values, needles = [], [], []
...
def _generate_samples(config: DictConfig) -> list[dict[str, any]]:
    ...
    if config.task_complexity.type_haystack == "essay":
        ...
        raise NotImplementedError(
            f"{config.task_complexity.type_haystack} is not implemented."
        )
...
def _app(config: DictConfig):
    ...
    config.task_complexity.num_needle_k = max(
        config.task_complexity.num_needle_k, config.task_complexity.num_needle_q
    )
